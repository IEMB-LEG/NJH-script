import pandas as pd
import matplotlib.pyplot as plt
import argparse
import numpy as np
from matplotlib.backends.backend_pdf import PdfPages

def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="Generate and display a stacked bar chart with adjusted length categories.")
    parser.add_argument("input_file", type=str, help="Path to the input file.")
    parser.add_argument("output_file", type=str, help="Path to the output PDF file.")
    parser.add_argument("--interval", type=int, default=10, help="Interval for length categories.")
    parser.add_argument("--max_length", type=int, default=70, help="Maximum length for categorization before lumping into a final category.")
    return parser.parse_args()

def read_data(input_file):
    """Read the input file into a DataFrame."""
    df = pd.read_csv(input_file, sep="\t", header=None, names=["Length", "FirstCol", "SecondCol"])
    return df

def categorize_data(df, interval, max_length):
    """Categorize length data and calculate counts."""
    # Adjust the bins to start from 1 and include an 'overflow' bin for lengths > max_length
    bins = np.arange(1, max_length, interval).tolist() + [max_length, df['Length'].max() + 1]
    labels = [f"{i}-{i+interval-1}" for i in range(1, max_length, interval)] + [f"{max_length}+"]
    df['Category'] = pd.cut(df['Length'], bins=bins, labels=labels, right=False)

    total_yes = df[df['FirstCol'] == "Yes"].groupby('Category').size()
    double_yes = df[(df['FirstCol'] == "Yes") & (df['SecondCol'] == "Yes")].groupby('Category').size()

    summary = pd.DataFrame({'TotalYes': total_yes, 'DoubleYes': double_yes}).fillna(0)
    return summary

def plot_data(summary, output_file):
    """Plot the data, save as PDF, and display the plot."""
    with PdfPages(output_file) as pdf:
        ax = summary.plot(kind='bar', stacked=True, figsize=(10, 6))
        plt.title('Stacked Bar Chart of Yes Counts')
        plt.xlabel('Length Category')
        plt.ylabel('Count')
        plt.xticks(rotation=45)
        plt.legend(['Total Yes', 'Both Columns Yes'])
        plt.tight_layout()
        pdf.savefig()
        plt.show()  # Display the plot

def main():
    args = parse_args()
    df = read_data(args.input_file)
    summary = categorize_data(df, args.interval, args.max_length)
    plot_data(summary, args.output_file)
    print(f"Stacked bar chart saved to {args.output_file} and displayed.")

if __name__ == "__main__":
    main()

