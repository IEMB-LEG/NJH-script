...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Generate and display a stacked bar chart with adjusted length categories and a configurable Y-axis limit.")
    ...
    parser.add_argument("--y_max", type=int, default=None, help="Maximum value for the y-axis.")  # Add command line argument for Y-axis max
...
def categorize_data(df, interval, max_length):
    """Categorize length data and calculate counts."""
    ...
    total_yes = df[df['FirstCol'] == "Yes"].groupby('Category').size()
    double_yes = df[(df['FirstCol'] == "Yes") & (df['SecondCol'] == "Yes")].groupby('Category').size()

    summary = pd.DataFrame({'TotalYes': total_yes, 'DoubleYes': double_yes}).fillna(0)
    return summary

def plot_data(summary, output_file, y_max):
    """Plot the data with a specified Y-axis limit, save as PDF, and display the plot."""
    with PdfPages(output_file) as pdf:
        ax = summary.plot(kind='bar', stacked=True, figsize=(10, 6))
        plt.title('Stacked Bar Chart of Yes Counts')
        plt.xlabel('Length Category')
        plt.ylabel('Count')
        plt.xticks(rotation=45)
        plt.legend(['Total Yes', 'Both Columns Yes'])
        if y_max is not None:  # Check if Y-axis max is specified
            plt.ylim(0, y_max)
        plt.tight_layout()
        pdf.savefig()
        plt.show()
...
def main():
    ...
    print(f"Stacked bar chart saved to {args.output_file} and displayed.")
